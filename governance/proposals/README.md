...
2. **Snapshot vote (3 days):** Off-chain signal vote among veAGENTIC holders
...
After Snapshot vote passes, an executor (any veAGENTIC holder) submits the on-chain proposal to the Timelock. The Timelock enforces a 48h delay. After delay, anyone can call `execute()` to apply the change.
